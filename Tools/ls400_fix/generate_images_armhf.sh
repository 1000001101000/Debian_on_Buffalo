...
for package in bsdmainutils mtd-utils ethtool libbsd0
do
  eth_deb_url="$(zcat Packages.gz | grep $package | grep -m 1 Filename | gawk '{print $2}')"
...
wget -N https://openwrt.org/_media/media/buffalo/ls421de/u-boot.buffalo-1.34_voodoo.tar.gz
tar xf u-boot.buffalo-1.34_voodoo.tar.gz
boot_check="$(md5sum u-boot.buffalo-1.34_voodoo | cut -d " " -f 1)"
if [ "$boot_check" != "42f7b0fa5a56389f13cf605ceec14809" ]; then
   echo "u-boot checksum failed, quitting"
   exit 99
fi
...
cp -v armhf-files/u-boot.buffalo-1.34_voodoo armhf-payload/source/
