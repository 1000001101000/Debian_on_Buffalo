...
kernel_ver="$(zcat initrd.gz | cpio -t | grep -m 1 lib/modules/ | gawk -F/ '{print $NF}')"
...
cd ..
mkdir armhf-payload/usr/lib/ 2>/dev/null
rm -r armhf-payload/usr/lib/modules/* 2>/dev/null
ln -s /usr/lib/ armhf-payload/lib
...
mkimage -A arm -O linux -T ramdisk -C gzip -a 0x0 -e 0x0 -n initrd -d initrd.xz output/initrd.buffalo.armhf
if [ $? -ne 0 ]; then
...
for dtb in $dtb_list
do
model="$(echo $dtb | gawk -F[-.] '{print $(NF-1)}')"
